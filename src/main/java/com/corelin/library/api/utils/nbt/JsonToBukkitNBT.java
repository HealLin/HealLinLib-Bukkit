package com.corelin.library.api.utils.nbt;

import com.corelin.library.exception.NBTException;
...
/**
 * @author 择忆霖心
 * @简述 将json数据转换成Bukkit的NBTTagCompound
 * @时间 2020/11/22 21:49
 * @版本 1.0
 */
public class JsonToBukkitNBT {


    private static final Pattern INT_ARRAY_MATCHER = Pattern.compile("\\[[-+\\d|,\\s]+\\]");

    /**
     * 将json数据转换成Bukkit的NBTTagCompound
     * @param jsonString JSON数据
     * @return NBTTagCompound类
     * @throws NBTException
     */
    ...
    static JsonToBukkitNBT.Any joinStrToNBT(String... args) throws NBTException
    {
        return nameValueToNBT(args[0], args[1]);
    }

    static JsonToBukkitNBT.Any nameValueToNBT(String key, String value) throws NBTException
    {
        value = value.trim();
        ...
        if (value.startsWith("{"))
        {
            ...
            JsonToBukkitNBT.Compound jsontonbt$compound;
            String s1;

            for (jsontonbt$compound = new JsonToBukkitNBT.Compound(key); value.length() > 0; value = value.substring(s1.length() + 1))
            {
        ...
        {
            value = value.substring(1, value.length() - 1);
            JsonToBukkitNBT.List jsontonbt$list;
            String s;

            for (jsontonbt$list = new JsonToBukkitNBT.List(key); value.length() > 0; value = value.substring(s.length() + 1))
            {
        ...
        {
            return new JsonToBukkitNBT.Primitive(key, value);
        }
    }

    private static JsonToBukkitNBT.Any getTagFromNameValue(String str, boolean isArray) throws NBTException
    {
    ...
    static class Compound extends JsonToBukkitNBT.Any
    {
        protected java.util.List<Any> tagList = Lists.<JsonToBukkitNBT.Any>newArrayList();
        ...
        public NBTBase parse() throws NBTException
        {
            ...
            for (JsonToBukkitNBT.Any jsontonbt$any : this.tagList)
            {
    ...
    static class List extends JsonToBukkitNBT.Any
    {
        protected java.util.List<Any> tagList = Lists.<JsonToBukkitNBT.Any>newArrayList();
    ...
    static class Primitive extends JsonToBukkitNBT.Any
    {

...
public class OrgJsonToNBT {
    ...
    static OrgJsonToNBT.Any joinStrToNBT(String... args) throws NBTException
    {
        return nameValueToNBT(args[0], args[1]);
    }

    static OrgJsonToNBT.Any nameValueToNBT(String key, String value) throws NBTException
    {
        value = value.trim();
        ...
        if (value.startsWith("{"))
        {
            ...
            OrgJsonToNBT.Compound jsontonbt$compound;
            String s1;

            for (jsontonbt$compound = new OrgJsonToNBT.Compound(key); value.length() > 0; value = value.substring(s1.length() + 1))
            {
        ...
        {
            value = value.substring(1, value.length() - 1);
            OrgJsonToNBT.List jsontonbt$list;
            String s;

            for (jsontonbt$list = new OrgJsonToNBT.List(key); value.length() > 0; value = value.substring(s.length() + 1))
            {
        ...
        {
            return new OrgJsonToNBT.Primitive(key, value);
        }
    }

    private static OrgJsonToNBT.Any getTagFromNameValue(String str, boolean isArray) throws NBTException
    {
    ...
    static class Compound extends OrgJsonToNBT.Any
    {
        protected java.util.List<Any> tagList = Lists.<OrgJsonToNBT.Any>newArrayList();
        ...
        public NBTBase parse() throws NBTException
        {
            ...
            for (OrgJsonToNBT.Any jsontonbt$any : this.tagList)
            {
    ...
    static class List extends OrgJsonToNBT.Any
    {
        protected java.util.List<Any> tagList = Lists.<OrgJsonToNBT.Any>newArrayList();
    ...
    static class Primitive extends OrgJsonToNBT.Any
    {
